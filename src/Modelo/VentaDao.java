...
public class VentaDao {
    ...
    public int RegistrarVenta(Venta v){
        String sql = "INSERT INTO ventas (cliente, usuario, total, fecha) VALUES (?,?,?,?)";
        try {
            con = cn.getConnection();
            ps = con.prepareStatement(sql);
    ...
    public void pdfV(int idventa, int Cliente, double total, String usuario) {
        try {
            ...
            doc.open();
            Image img = Image.getInstance(getClass().getResource("/Img/farmacia.png"));
            //Fecha
            Paragraph fecha = new Paragraph();
            Font negrita = new Font(Font.FontFamily.TIMES_ROMAN, 12, Font.BOLD, BaseColor.BLUE);
            fecha.add(Chunk.NEWLINE);
            fecha.add("Usuario: " + usuario + "\nFolio: " + idventa + "\nFecha: "
            ...
            Encabezado.addCell("");
            //info empresa
            String config = "SELECT * FROM name_farmacia";
            String mensaje = "";
            try {
                con = cn.getConnection();
                ...
                if (rs.next()) {
                    ...
                    Encabezado.addCell("RFC:    " + rs.getString("suc") + "\nNombre: " + rs.getString("nombre") + "\nTeléfono: " + rs.getString("telefono") + "\nDirección: " + rs.getString("direccion") + "\n\n");
                }
            } catch (SQLException e) {
                System.out.println(e.toString());
            }
